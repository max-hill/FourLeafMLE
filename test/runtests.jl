...
test_random_binary_quartet_model(:uniform) # run once to avoid compilation problems

@testset "Is estimation accurate for random trees in the interior?" begin
    # The easy test
    test_result1 = [test_random_binary_quartet_model(:uniform) for i in 1:10]
    @test all(==(nothing),test_result1)

    # The harder test by randomly choosing branch lengths closer to the boundary
    # (see commentary below).
    alpha, beta = 0.6, 0.6
    skewed_distribution = Beta(alpha,beta)
    test_result2 = [test_random_binary_quartet_model(skewed_distribution) for i in 1:10]
    # Estimation failure rate in the above test appears to be <1%. 
    @test sum([x==nothing for x in test_result2])/length(test_result2) >= .90
...
@testset "Is estimation accurate for random trees on the boundary?" begin
    ...
    for j in 1:3
        test_result[j] = [test_random_infinite_branch_model(j) for i in 1:10]
        @test all(==(nothing),test_result[j])
    end
    
    # Uniformly-chosen branch lengths with 1-2 length zero branches
    test_result4 = [test_random_zero_branch_model() for i in 1:10] 
    ...
    test_result5 = [test_random_zero_branch_model(skewed_distribution) for i in 1:10]
    @test sum([x==nothing for x in test_result5])/length(test_result5) >= .90
...
# Is non-generic data handled correctly?                   |    1      1  8.7s
# Is estimation accurate for random trees in the interior? |    2      2  6m58.4s
# Is estimation accurate for random trees on the boundary? |    5      5  17m04.4s
